...
<div class="tabber" id="settingstabber">
  <div class="tabbertab" title="Basic settings">
<form method="post">
<table class="formatted infotable">
<tr><td colspan="2" class="tabletitle">Registration</td></tr>
<tr><td>Extra information when registering</td><td><textarea name="registrationAddition" cols="60" rows="4"><%=settings.getRegistrationAddition() %></textarea></td></tr>
<tr><td>Send confirmation e-mail after registering</td><td><input name="sendConfirmationEmailAfterRegistration" type="checkbox" <%=settings.isSendConfirmationEmailAfterRegistration() ? " checked=\"checked\"" : "" %>></input></td></tr>
<tr><td>Allow self registration</td><td><input type="checkbox" name="allowSelfRegistration" <%=settings.isAllowSelfRegistration() ? " checked=\"checked\"" : "" %>></input></td></tr>
<tr><td colspan="2" class="tabletitle">E-mail</td></tr>
<tr><td>Address of e-mail sender</td><td><input type="text" name="emailsenderaddress" value="<%=settings.getEmailSenderAddress()%>"></input></td></tr>
<tr><td>SMTP server (for outgoing e-mail)</td><td><input name="smtpServer" type="text" value="<%=settings.getSmtpServer()%>"></input></td></tr>
<tr><td colspan="2" class="tabletitle">Database</td></tr>
<tr><td>Database location</td><td><input type="text" name="databaselocation" value="<%=settings.getDatabaseLocation()%>"></input></td></tr>
<tr><td colspan="2" class="tabletitle">Other</td></tr>
<tr><td>Show whether a new version is available</td><td><input name="showVersionUpgradeAvailable" type="checkbox" <%=settings.isShowVersionUpgradeAvailable() ? " checked=\"checked\"" : "" %>></input></td></tr>
<tr><td>Use second JVM for IFCEngine</td><td><input name="usesecondenginejvm" type="checkbox" <%=settings.isUseSecondEngineJvm() ? " checked=\"checked\"" : "" %>></input></td></tr>
<!-- 
<tr><td>Use file-level caching</td><td><input name="usecaching" type="checkbox" <%=settings.isUseCaching() ? " checked=\"checked\"" : "" %>></input></td></tr>
 -->
<tr><td>Enabled export types</td><td>
<table class="cleantable">
<%
	for (ResultType resultType : ResultType.values()) {
		if (resultType.isUserType()) {
%>
<tr><td><label for="<%=resultType.name()%>"><%=resultType.getNiceName() %></label></td><td><input name="<%=resultType.name() %>" id="<%=resultType.name() %>" type="checkbox"<%=emfSerializerFactory.getSingleResultTypes().contains(resultType) ? " checked=\"checked\"" : "" %>></input></td></tr>
<%
		}
	}
%>
...
<tr><td></td><td><input name="save" type="submit" value="Save"></input></td></tr>
</table>
</form>
...
<table class="formatted infotable">
<tr><td>Download</td><td><a href="<%=getServletContext().getContextPath() %>/settings?action=downloadsettings">Download settings.xml</a></td></tr>
<tr><td>Upload</td><td><input type="file" name="settings"></input><input type="submit" value="Upload"></input></td></tr>
</table>
<input type="hidden" name="action" value="uploadsettings"/>
</form>
</div>
  <div class="tabbertab" title="Ignore file">
<form enctype="multipart/form-data" method="post" action="<%=getServletContext().getContextPath() %>/settings">
<table class="formatted infotable">
<tr><td>Download</td><td><a href="<%=getServletContext().getContextPath() %>/settings?action=downloadignorefile">Download ignore.xml</a></td></tr>
<tr><td>Upload</td><td><input type="file" name="ignorefile"></input><input type="submit" value="Upload"></input></td></tr>
</table>
<input type="hidden" name="action" value="uploadsettings"/>
</form>
  </div>
