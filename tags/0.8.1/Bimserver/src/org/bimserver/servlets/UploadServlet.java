	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isMultipart) {
			...
			try {
				...
				if (pid != -1) {
					if (size == 0) {
						response.sendRedirect("project.jsp?pid=" + pid + "&message=Uploaded file empty, or no file uploaded at all");
					} else {
						...
						if (type.equals("ifc")) {
							try {
								...
								loginManager.getService().checkin(pid, comment, size, ifcFile);
								...
								if (e.getCause() != null) {
									response.sendRedirect("project.jsp?pid=" + pid + "&message=" + e.getCause().getMessage());
								} else {
									response.sendRedirect("project.jsp?pid=" + pid + "&message=" + e.getUserMessage());
								}
							}
