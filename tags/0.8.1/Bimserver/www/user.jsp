...
%>
	</select> <label for="zip_<%=revision.getId() %>">Zip</label><input type="checkbox" name="zip" id="zip_<%=revision.getId() %>"/> <input type="submit" value="Download"/>
	</form>
	</td>
	<td>
	<form method="post" action="<%=request.getContextPath() %>/download">
...
<% if (nonAuthorizedProjects.size() > 0 && loginManager.getUserType().equals("ADMIN")) { %>
